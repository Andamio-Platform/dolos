...
Base: [`txpipe/dolos v1.6.0`](https://github.com/txpipe/dolos/releases/tag/v1.6.0)

Rebase of the andamio patch stack from the `v1.2.0` base onto `v1.6.0`. One
conflict during the rebase, in `crates/minibf/src/mapping.rs` (see below);
everything else applied cleanly via `git rebase` with no logic changes.
...
### REMOVED: `minibf` — address-utxo `tx_hash` from `TxoRef` *(superseded — now upstream)*

The `v1.2.0-andamio.2` fix sourcing `tx_hash` from the UTxO's own `TxoRef`
instead of the archive `block_data` lookup (see that section below) is
dropped on this base: upstream landed the identical fix independently
somewhere between `v1.2.0` and `v1.6.0`.

Confirmed during the rebase: the only conflict in this pass was in
`crates/minibf/src/mapping.rs`, and both sides — ours and upstream's —
produced byte-identical code (`tx_hash: self.txo_ref.0.to_string()`),
differing only in comment wording. Resolved in upstream's favor; the
resulting file is fully identical to stock `v1.6.0`, so this patch has zero
remaining diff to carry and is dropped rather than reapplied.
...
### `minibf` — source address-utxo `tx_hash` from `TxoRef` *(superseded — see v1.6.0-andamio.1)*
